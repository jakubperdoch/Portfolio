...
export default function ContactClient() {
  ...
  const mailto = `mailto:${email}?subject=${encodeURIComponent(t(topic))}&body=${encodeURIComponent(message)}`;
          ...
          <div className="mt-8 space-y-7">
            ...
            <div>
              <a
                ...
                className="font-heading inline-flex items-center gap-3 rounded-full bg-zinc-900 px-6 py-3.5 text-sm text-white transition-colors hover:bg-zinc-700 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-zinc-900"
              >
                {t("openEmail")}
                <ArrowUpRight size={18} aria-hidden="true" />
              </a>
              <p className="font-heading mt-3 text-sm leading-6 text-zinc-500">{t("emailHint")}</p>
              {opened && (
                <p role="status" className="mt-2 text-sm text-zinc-600">
                  {t("fallback")}
                </p>
